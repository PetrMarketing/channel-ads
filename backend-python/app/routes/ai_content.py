...
# Творительный падеж — для фразы «публикуется ___» (зимой/летом/etc)
_RU_SEASONS_TV = {"зима": "зимой", "весна": "весной", "лето": "летом", "осень": "осенью"}
...
def _build_user_prompt_single(
    ...
    if start_date:
        try:
            ...
            season_tv = _RU_SEASONS_TV.get(season, season)
            if forbidden:
                season_warning = (
                    f"⚠️ ЗАПРЕТ: пост публикуется {season_tv}. НЕ упоминай и НЕ "
                    f"обыгрывай темы из другого сезона ({forbidden}). "
                    f"Сезонные привязки — только к {season_tv}у периоду и конкретному месяцу "
                    f"({month_name})."
                )
        except Exception as e:
