        ...
        <div style={modalOverlay}>
          <div style={{ ...modalBox, maxWidth: 600 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: 17, fontWeight: 700 }}>
              {editing.id ? 'Изменить рассылку' : 'Новая рассылка'}
            </h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <BField label="Текст кнопки" v={editing.button_text} onChange={v => setEditing(e => ({ ...e, button_text: v }))} placeholder="Узнать подробнее" />
              <BField label="Ссылка кнопки" v={editing.button_url} onChange={v => setEditing(e => ({ ...e, button_url: v }))} placeholder="https://itcakes.ru/achievements" />
            </div>

            <div>
              <label style={fieldLabel}>Аудитория {audCount !== null && <span style={{ color: '#4361ee' }}>(≈ {audCount} получателей)</span>}</label>
            ...
            <BField label="Запланировать на" type="datetime-local" v={editing.scheduled_at?.slice(0, 16) || ''} onChange={v => setEditing(e => ({ ...e, scheduled_at: v || null }))} />
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', marginTop: 16, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => save('draft').then(id => id && setEditing(null))} style={btnOutline}>💾 Сохранить как черновик</button>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={sendTest} style={{ ...btnOutline, color: '#4361ee', borderColor: '#4361ee' }}>📨 Отправить себе</button>
                {editing.scheduled_at && (
                  <button onClick={schedule} style={{ ...btnPrimary, background: '#3b82f6' }}>⏰ Запланировать</button>
                )}
                <button onClick={sendNow} style={{ ...btnPrimary, background: '#16a34a' }}>📢 Отправить сейчас</button>
              </div>
            </div>
          </div>
        </div>
